...
export default function NotificationsScreen() {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        const response = await fetch(`http://192.168.145.91:8000/user-notifications/${userEmail}`);
  ...
  const deleteNotification = async (id) => {
    try {
      ...
      const response = await fetch(`http://192.168.145.91:8000/user-notifications/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
  ...
  const NotificationItem = ({ note, isLast }) => {
    ...
    const parseNotificationTime = (utcTimeString) => {
      // First, parse as UTC and then convert to local timezone
      const utcTime = dayjs.utc(utcTimeString);
      const localTime = utcTime.local();
      
      // Debug: uncomment these lines to see the difference
      // console.log('UTC Time:', utcTime.format());
      // console.log('Local Time:', localTime.format());
      // console.log('Difference in hours:', localTime.diff(utcTime, 'hour'));
      
      return localTime;
    };

    const localDate = parseNotificationTime(note.created_at);
        ...
        <Swipeable renderRightActions={() => renderRightActions(note._id)}>
          ...
          >
            <View>
              ...
              {/* Debug: uncomment to see the actual parsed time */}
              {/* <Text className="text-xs text-gray-400 mt-1">
                Debug: {localDate.format('YYYY-MM-DD HH:mm:ss')}
              </Text> */}
            </View>
          </Animated.View>
        </Swipeable>
